...
from peep.src.analysis_preprocessing import read_results_summary
from peep.src.file_utils import construct_filename_from_metadata, get_tag
from peep.src.zenodo_utils import (
    create_deposition,
    get_deposition_id,
...
def create_new_deposition(use_sandbox=False):
    ...
    print(
        "Created new deposition with deposition_id={}.  We recommend that you now copy this deposition_id into the {} section of peep/zenodo_config.py".format(
...
def upload_results_summary(json_location, json_location_type, use_test_api):
    """
    Upload the results summary json from running peep pipeline to download and process data from GEE.
    """
