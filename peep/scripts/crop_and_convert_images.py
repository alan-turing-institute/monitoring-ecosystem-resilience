...
def main():
    ...
    parser.add_argument(
        "--input_dir",
        help="full path to directory containing input images",
        required=True,
    )
    parser.add_argument(
        "--output_dir", help="directory to put output images", required=True
    )

    parser.add_argument(
        "--threshold",
        help="sum(r,g,b) threshold above which we colour the pixel white, or below black",
        default=470,
        type=int,
    )
    parser.add_argument(
        "--num_pix_x",
        help="Size in pixels of cropped image along x-axis",
        default=50,
        type=int,
    )
    parser.add_argument(
        "--num_pix_y",
        help="Size in pixels of cropped image along y-axis",
        default=50,
        type=int,
    )
