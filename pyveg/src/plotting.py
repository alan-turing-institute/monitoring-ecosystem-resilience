...
def plot_time_series(df, output_dir, filename_suffix=""):
    ...
    def make_plot(
        df, veg_prefix, output_dir, veg_prefix_b=None, smoothing_option="smooth"
    ):
        ...
        ax.set_ylim(
            [veg_means.min() - 1 * veg_std.max(), veg_means.max() + 3 * veg_std.max()]
        )
        ...
        if any([smoothing_option in c and veg_prefix in c for c in veg_df.columns]):
            ...
            veg_means_smooth = veg_df[
                veg_prefix + "_offset50_" + smoothing_option + "_mean"
            ]
            veg_stds_smooth = veg_df[
                veg_prefix + "_offset50_" + smoothing_option + "_std"
            ]
        ...
        if veg_prefix_b:
            ...
            textstr = (
                f"$r_{{vv}}={vegveg_corr_smooth:.2f}$ (${vegveg_corr:.2f}$ unsmoothed)"
            )
            ax2.text(
                0.55,
                0.85,
        ...
        ar1_dict = {}
        ar1_dict["AR1"] = {
            "unsmoothed": {"param": unsmoothed_ar1, "se": unsmoothed_ar1_se},
            "smoothed": {"param": smoothed_ar1, "se": smoothed_ar1_se},
        }
        ...
        kendall_tau_dict["Kendall_tau"] = {
            "unsmoothed": {"tau": tau, "p": p},
            "smoothed": {"tau": tau_smooth, "p": p_smooth},
        }
        write_to_json(
            os.path.join(output_dir, veg_prefix + "_stats.json"), kendall_tau_dict
        )
    ...
    if np.sum(df.columns.str.contains("offset50_mean")) == 2:
        veg_columns = df.columns[
            np.where(df.columns.str.contains("offset50_mean"))
        ].values
...
def plot_autocorrelation_function(df, output_dir, filename_suffix=""):
    ...
    def make_plots(series, output_dir, filename_suffix=""):
        ...
        # save the plot
        output_filename = (
            series.name + "-autocorrelation-function" + filename_suffix + ".png"
        )
        plt.savefig(os.path.join(output_dir, output_filename), dpi=DPI)
        plt.close(fig)
...
def plot_cross_correlations(df, output_dir):
    ...
    def make_plot(veg_ys, precip_ys, output_dir):
        ...
        write_to_json(
            os.path.join(output_dir, "lagged_correlations.json"), correlations_dict
        )
...
def plot_moving_window_analysis(df, output_dir, filename_suffix=""):
    ...
    def make_plot(df, column, output_dir, smoothing_option):
        ...
        collection_prefix = (
            column.split("_")[0] if "offset50" in column else "precipitation"
        )
        ...
        if any([smoothing_option in c for c in df.columns]):
            variance_smooth = var_df[
                column.replace(
                    "offset50_mean", "offset50_" + smoothing_option + "_mean"
                )
            ]
            ar1_smooth = ar1_df[
                column.replace("var", "ar1").replace(
                    "offset50_mean", "offset50_" + smoothing_option + "_mean"
                )
            ]
            ar1_se_smooth = ar1_df[
                column.replace("var", "ar1_se").replace(
                    "offset50_mean", "offset50_" + smoothing_option + "_mean"
                )
            ]
        ...
        # plot variance
        ax2.plot(
            var_xs, variance, linewidth=2, color=color, alpha=0.75, label="Variance"
        )
        if any([smoothing_option in c for c in df.columns]):
            ax2.plot(
                var_xs,
    ...
    for column in df.columns:
        if (
            "offset50_mean" in column or "total_precipitation" in column
        ) and "var" in column:
...
def plot_ews_resiliance(series_name, EWSmetrics_df, Kendalltau_df, dates, output_dir):
    ...
    def annotate(text, xy=(6, 70), size=10):
        if "Kendall" in text:
            xy = (xy[0], 60)
        plt.gca().annotate(
            text, xy=xy, xycoords="axes points", size=size, ha="left", va="top"
        )
    ...
    plt.plot(
        dates[-len(ys) :], EWSmetrics_df["Smoothing"], color="red", linestyle="dashed"
    )
...
def plot_sensitivity_heatmap(series_name, df, output_dir):
    ...
    for column in df.columns:
        if column == "smooth" or column == "winsize":
            ...
            piv = pd.pivot_table(
                df, values=column, index=["smooth"], columns=["winsize"], fill_value=0
            )
            ...
            output_filename = (
                series_name.replace(" ", "-") + "-" + column + "-sensitivity.png"
            )
