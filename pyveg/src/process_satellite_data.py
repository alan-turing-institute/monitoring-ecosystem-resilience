...
def process_sub_image(i, sub, sub_rgb, sub_ndvi, output_subdir, date):
    ...
    if not check_image_ok(colour_subimage):
        ...
        save_image(
            colour_subimage, os.path.join(output_subdir, "rejected"), output_filename
        )
        return
    ...
    save_json(
        nc_result,
        os.path.join(output_subdir, "tmp_json"),
...
def run_network_centrality(
    ...
    sub_images = crop_image_npix(
        img_thresh, sub_image_size[0], sub_image_size[1], region_size, coords
    )

    sub_images_rgb = crop_image_npix(
        img_rgb, sub_image_size[0], sub_image_size[1], region_size, coords
    )

    sub_images_ndvi = crop_image_npix(
        ndvi_img, sub_image_size[0], sub_image_size[1], region_size, coords
    )
    ...
    nc_results = consolidate_json_to_list(
        os.path.join(output_subdir, "tmp_json"),
        output_subdir,
...
def get_vegetation(
    ...
    download_path, log_msg = ee_download(
        output_dir, collection_dict, coords, date_range, region_size, scale
    )

    # save the rgb image
    filenames = [
        filename for filename in os.listdir(download_path) if filename.endswith(".tif")
    ]
    ...
    rgb_filepath = construct_image_savepath(
        output_dir, collection_dict["collection_name"], coords, date_range, "RGB"
    )
    save_image(rgb_image, os.path.dirname(rgb_filepath), os.path.basename(rgb_filepath))

    # save the NDVI image
    ndvi_image = scale_tif(tif_filebase, "NDVI")
    ndvi_filepath = construct_image_savepath(
        output_dir, collection_dict["collection_name"], coords, date_range, "NDVI"
    )
    save_image(
    ...
    ndvi_bw_filepath = construct_image_savepath(
        output_dir, collection_dict["collection_name"], coords, date_range, "BWNDVI"
    )
    save_image(
        processed_ndvi,
    ...
    if collection_dict["do_network_centrality"]:
        ...
        nc_results = run_network_centrality(
            nc_output_dir,
            processed_ndvi,
            rgb_image,
            ndvi_image,
    ...
    for file in os.listdir(download_path):
        if file.endswith(".tif"):
            ...
            variable_array = cv.imread(
                os.path.join(download_path, file), cv.IMREAD_ANYDEPTH
            )
...
def process_single_collection(
    output_dir, collection_dict, coords, date_ranges, region_size=0.1, scale=10
):
    """
    ...
    output_subdir = os.path.join(
        output_dir, collection_dict["collection_name"].replace("/", "-")
    )
    ...
    for date_range in date_ranges:
        ...
        if collection_dict["type"] == "vegetation":
            result = get_vegetation(
                output_subdir, collection_dict, coords, date_range, region_size, scale
            )
        else:
            result = get_weather(
                output_subdir, collection_dict, coords, date_range, region_size, scale
...
def process_all_collections(
    output_dir, collections, coords, date_range, region_size=0.1, scale=10
):
    """
    ...
    for _, collection_dict in collections.items():  # possible to parallelise?
        ...
        date_ranges = slice_time_period(
            start_date, end_date, str(num_days_per_point) + "d"
        )  # pass directly the time interval

        # get the data
        results = process_single_collection(
            output_dir, collection_dict, coords, date_ranges, region_size, scale
        )
