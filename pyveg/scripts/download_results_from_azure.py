...
def get_rgb(container, rgb_dir):
    ...
    bbs = BlockBlobService(account_name=config["storage_account_name"],
...
def create_zip_archive(temp_dir, output_zipname):
    ...
    subprocess.run(["cp", os.path.join(temp_dir, os.path.basename(output_zipname)),
                    output_zipname])


def main():
    parser = argparse.ArgumentParser(description="download from blob storage to zipfile")
    parser.add_argument("--container", help="name of blob storage container", required=True)
    parser.add_argument("--output_zipfile", help="name of zipfile to write to", default="./results.zip")
