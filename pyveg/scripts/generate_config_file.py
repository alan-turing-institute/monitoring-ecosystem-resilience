...
def get_template_text():
    ...
    if not os.path.exists(template_filepath):
        raise RuntimeError(
            "Unable to find template {}".format(template_filepath))
...
def main():
    ...
    while not os.path.exists(configs_dir):
        if os.path.exists(os.path.join("pyveg", "configs")):
            ...
            default_configs_dir = "."
        configs_dir = input(
            "Enter path to directory containing config files, or press Return for default path ('{}') : ".format(default_configs_dir))
    ...
    while not collection_name in collection_names:
        collection_name = input(
            "Please enter a valid collection name from this list: {} : ".format(collection_names))
    ...
    if not latitude:
        ...
        while not (isinstance(latitude, float) and latitude > lat_range[0] and latitude < lat_range[1]):
            latitude = float(
                input("please enter Latitude (degrees N) in the range {} : ".format(lat_range)))
    if not longitude:
        ...
        while not (isinstance(longitude, float) and longitude > long_range[0] and longitude < long_range[1]):
            longitude = float(input(
                "please enter Longitude (degrees E) in the range {} : ".format(long_range)))
    ...
    while not date_regex.search(start_date):
        start_date = input(
            "Enter start date in format YYYY-MM-DD, or press Return for default ({}) : ".format(default_start_date))
        if len(start_date) == 0:
    ...
    while not date_regex.search(end_date):
        end_date = input(
            "Enter end date in format YYYY-MM-DD, or press Return for default ({}) : ".format(default_end_date))
        if len(end_date) == 0:
    ...
    while not time_per_point_regex.search(time_per_point):
        time_per_point = input(
            "Enter time per point in format e.g. '1m' for 1 month, '1w' for 1 week, or press Return for default ({}) : ".format(default_time_per_point))
    ...
    if not region_size:
        ...
        while not (isinstance(region_size, float) and region_size > 0. and region_size <= 0.08):
            region_size = input(
                "Enter region size in degrees latitude/longitude, or press Return for max/default ({}) : ".format(default_region_size))
    ...
    if not pattern_type:
        ...
        while len(pattern_type) < 1:
            pattern_type = input(
                "Enter type of patterned vegetation (e.g. 'spots', 'labyrinths', or press Return for default ('{}') : ".format(default_pattern_type))
    ...
    while not run_mode in run_modes:
        run_mode = input(
            "Would you like time-consuming functions to be run on the cloud?  Choose from the following: {}, or press Return for default option '{}': ".format(run_modes, default_run_mode))
        if len(run_mode) == 0:
    ...
    while not (isinstance(n_threads, int) and n_threads in n_threads_range):
        if run_mode == "local":
            n_threads = input(
                "How many threads would you like time-consuming processing functions to use?  (Many computers will have 4 or 8 threads available).  Press return for default value {} : ".format(default_n_threads))
            if len(n_threads) == 0:
