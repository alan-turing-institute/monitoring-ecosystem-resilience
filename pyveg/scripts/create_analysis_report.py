...
def get_collection_and_suffix(collection_name):
    ...
    if collection_name == 'COPERNICUS/S2':
        ...
        raise RuntimeError(
            "Unknown collection_name {}".format(collection_name))
...
def add_time_series_plots(mdFile, analysis_plots_location, analysis_plots_location_type, satellite_suffix):
    ...
    fig_count += 1
    mdFile.new_line(mdFile.new_inline_image(text='Time series Offset50', path=os.path.join(
        ts_path, satellite_suffix+'-time-series_smooth.png')))
    #mdFile.new_paragraph('Figure '+str(count)+': '+'Time series Offset50')

    fig_count += 1
    mdFile.new_line(mdFile.new_inline_image(text='Time series NDVI', path=os.path.join(
        ts_path, satellite_suffix+'-ndvi-time-series.png')))
    #mdFile.new_paragraph('Figure '+str(count)+': '+'Time series NDVI')
    mdFile.new_line('')

    # STL figures
    mdFile.new_header(level=1, title='STL decomposition')
    stl_path = os.path.join(analysis_plots_location,
                            'analysis', 'detrended', 'STL')
    fig_count += 1

    mdFile.new_line(mdFile.new_inline_image(text='STL Offset50', path=os.path.join(
        stl_path, satellite_suffix+'_offset50_mean_STL_decomposition.png')))
    #mdFile.new_paragraph('Figure '+str(count)+': '+'STL Offset50')
    fig_count += 1
    mdFile.new_line(mdFile.new_inline_image(text='STL NDVI', path=os.path.join(
        stl_path, satellite_suffix+'_ndvi_mean_STL_decomposition.png')))
    ...
    if os.path.exists(os.path.join(analysis_plots_location, 'analysis', 'resiliance', 'deseasonalised', 'ewstools')):
        ...
        ews_path = os.path.join(
            analysis_plots_location, 'analysis', 'resiliance', 'deseasonalised', 'ewstools')
        fig_count += 1
        mdFile.new_line(mdFile.new_inline_image(text='EWS Offset50', path=os.path.join(
            ews_path, satellite_suffix+'-offset50-mean-ews.png')))
...
def add_rgb_images(mdFile, rgb_location, rgb_location_type, fig_count):
    ...
    mdFile.new_header(level=1, title='RGB images through time')
    rgb_filenames = []
        ...
        for root, dirs, files in os.walk(rgb_location):
            for filename in files:
                if filename.endswith("RGB.png") and os.path.basename(filename).startswith('sub') == False:
                    rgb_filenames.append(os.path.join(
                        rgb_location, root, filename))
    elif rgb_location_type == "azure":
        tmpdir = tempfile.mkdtemp()
        download_rgb(rgb_location, tmpdir)
        rgb_filenames = [os.path.join(tmpdir, fname)
    ...
    for i, rgb_figure in enumerate(rgb_filenames):
        ...
        mdFile.new_line(mdFile.new_inline_image(
            text=rgb_figure, path=rgb_figure))
...
def create_markdown_pdf_report(analysis_plots_location,
    ...
    if not metadata:
        if os.path.exists(os.path.join(analysis_plots_location, "results_summary.json")):
            try:
                metadata = json.load(
                    open(os.path.join(analysis_plots_location, "results_summary.json")))["metadata"]
            except:
                print("Couldn't retrieve metadata from {}".format(
                    analysis_plots_location, "results_summary.json"))
        elif rgb_location_type == "azure":
            tmpdir = tempfile.mkdtemp()
            download_summary_json(rgb_location, tmpdir)
            try:
                metadata = json.load(open(os.path.join(tmpdir, "results_summary.json")))[
                    "metadata"]
            except:
                print("Couldn't retrieve metadata from {}".format(
                    tmpdir, "results_summary.json"))
    if metadata and not collection_name:
        collection_name = metadata['collection']
    if not collection_name:
        raise RuntimeError(
            "please provide either a metadata dictionary or a collection name (e.g. 'COPERNICUS/S2')")
    ...
    if not coordinates:
        raise RuntimeError(
            "Unable to find coordinates from path.  Please provide a metadata dict.")

    if not output_dir:
        # put the output report in the same directory as the analysis plots
        output_dir = analysis_plots_location
    os.makedirs(output_dir, exist_ok=True)
    output_path = os.path.join(output_dir, 'analysis_report_{}_{}_{}'.format(
        coordinates[0], coordinates[1], collection))
    ...
    output = pypandoc.convert_file(
        output_path+'.md', 'pdf', outputfile=output_path+".pdf")
    assert output == ""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Collect all figures from analysis and get them into report")
    parser.add_argument("--input_analysis_plots_location",
                        help="results directory from `download_gee_data` script, containing `results_summary.json` and `analysis` directory")
    parser.add_argument("--input_analysis_plots_location_type",
                        help="currently supports 'local' or 'azure'", default="local")
    parser.add_argument("--input_rgb_location",
                        help="location of the RGB plots - either an Azure container or a local directory")
    parser.add_argument("--input_rgb_location_type",
                        help="currently supports 'local' or 'azure'", default="local")
    parser.add_argument(
        "--output_dir", help="(optional) directory to store output report.  If not specified, will use input_analysis_plots_location")
    parser.add_argument("--do_timeseries", action="store_true",
                        help="include time-series plots in the report")
    parser.add_argument(
        "--collection", help="Satellite to be used in the report ", default='COPERNICUS/S2')
