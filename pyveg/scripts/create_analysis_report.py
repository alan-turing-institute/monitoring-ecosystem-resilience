...
def add_time_series_plots(
    mdFile, analysis_plots_location, analysis_plots_location_type, satellite_suffix
):
    ...
    mdFile.new_line(
        mdFile.new_inline_image(
            text="Time series Offset50",
            path=os.path.join(ts_path, satellite_suffix + "-time-series_smooth.png"),
        )
    )
    ...
    mdFile.new_line(
        mdFile.new_inline_image(
            text="Time series NDVI",
            path=os.path.join(ts_path, satellite_suffix + "-ndvi-time-series.png"),
        )
    )
    ...
    if os.path.exists(
        ...
        mdFile.new_line(
            mdFile.new_inline_image(
                text="EWS NDVI",
                path=os.path.join(ews_path, satellite_suffix + "-ndvi-mean-ews.png"),
            )
        )
        mdFile.new_line("")
    ...
    mdFile.new_line(
        mdFile.new_inline_image(
            text="Offset50 CB fit",
            path=os.path.join(
                analysis_plots_location, "analysis", "fit_ts_CB_S2_offset50_mean.png"
            ),
        )
    )
    mdFile.new_line("")
    ...
    mdFile.new_line(
        mdFile.new_inline_image(
            text="NDVI CB fit",
            path=os.path.join(
                analysis_plots_location, "analysis", "fit_ts_CB_S2_ndvi_mean.png"
            ),
        )
    )
    mdFile.new_line("")
    ...
    mdFile.new_line(
        mdFile.new_inline_image(
            text="total precipitation CB fit",
            path=os.path.join(
                analysis_plots_location, "analysis", "fit_ts_CB_total_precipitation.png"
            ),
        )
    )
    mdFile.new_line("")
    ...
    if rgb_location_type == "local":
        for root, dirs, files in os.walk(rgb_location):
            for filename in files:
                if (
                    filename.endswith("RGB.png")
                    and os.path.basename(filename).startswith("sub") == False
                ):
...
def create_markdown_pdf_report(
    analysis_plots_location,
    ...
    if not metadata:
        if os.path.exists(
            os.path.join(analysis_plots_location, "results_summary.json")
        ):
            try:
                metadata = json.load(
                    open(os.path.join(analysis_plots_location, "results_summary.json"))
                )["metadata"]
            except:
                print(
                    "Couldn't retrieve metadata from {}".format(
            ...
            try:
                metadata = json.load(
                    open(os.path.join(tmpdir, "results_summary.json"))
                )["metadata"]
            except:
                print(
                    "Couldn't retrieve metadata from {}".format(
    ...
    output_path = os.path.join(
        output_dir,
        "analysis_report_{}_{}_{}".format(coordinates[0], coordinates[1], collection),
    )

    # create the markdown file
    mdFile = MdUtils(
        file_name=output_path,
        title="Results for {} and coordinates {} (longitude) and {} (latitude)".format(
            collection, coordinates[0], coordinates[1]
        ),
    )

    fig_count = 0
    if do_timeseries:
        mdFile, fig_count = add_time_series_plots(
            mdFile,
            analysis_plots_location,
            analysis_plots_location_type,
            satellite_suffix,
...
def main():
    ...
    parser.add_argument(
        ...
        help="currently supports 'local' or 'azure'",
        default="local",
    )
    parser.add_argument(
        "--input_rgb_location",
    ...
    parser.add_argument(
        "--output_dir",
        help="(optional) directory to store output report.  If not specified, will use input_analysis_plots_location",
    )
    parser.add_argument(
        "--do_timeseries",
        action="store_true",
    ...
    parser.add_argument(
        "--collection",
        help="Satellite to be used in the report ",
