...
def convert_gee_date(gee_date_series):
    ...
    dates = [
        datetime.datetime.strptime(d, "%b %d, %Y").strftime("%Y-%m-%d")
        for d in gee_date_series
    ]
...
def convert_gee_outputs(input_dir):
    ...
    files = [f for f in os.listdir(input_dir) if "ee-chart" in f and ".csv" in f]

    # check some relevant files exist
    if len(files) == 0:
        raise FileNotFoundError(
            (f"Could not find any compatible csv " 'files in "{input_dir}"! Exiting.')
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run pyveg analysis on data download from the GEE Editor."
    )
    parser.add_argument(
        "--input_dir",
        help="Directory containing files corresponding to different downloaded timeseries.",
    )
